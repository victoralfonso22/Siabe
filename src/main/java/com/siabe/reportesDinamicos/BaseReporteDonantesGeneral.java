...
public abstract class BaseReporteDonantesGeneral extends TestCase {
	
	@Autowired
	DonativosServicio donativosServicio;

	public Map getParams() {
        return params;
    }

    protected static final Log log = LogFactory.getLog(BaseReporteDonantesGeneral.class);
    ...
    protected JRDataSource getDataSource() {
    	...
    	List<Donativos> don = donativosServicio.reporteGeneral(idPeriodo, idRegion);
    	
    	for(int a = 0;a< don.size();a++) {
    		don.get(a).setIdDonativo(a+1);
    	}
       ...
       /* Collection<Donativos> collection = new ArrayList<Donativos>();
        
        Iterator<Donativos> iterator = dummyCollection.iterator();
        int a = 1;
    ...
    /*public Collection getDummyCollectionSorted(List columnlist) {
        Collection dummyCollection = beneficiariosServicio.todosBeneficiarios();
